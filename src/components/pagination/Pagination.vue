<template>
  <div class="card w-100">
    <div class="card-body m-0 p-3 d-flex justify-content-between align-items-center" id="card-body">
        ...
        <div>Page <span>{{ data.currentPage }} of {{ data.totalPages }}</span>
        </div>
      <nav aria-label="...">
        <ul class="pagination">
          <li class="page-item" :class="{ 'disabled': data.currentPage == 1 }">
            <a class="page-link" href="#" @click.prevent="prevPage()" tabindex="-1" aria-disabled="true"
              >Previous</a
            >
          </li>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const totalPages = computed(() => Math.ceil(props.data.totalRecords / props.data.perPage))
