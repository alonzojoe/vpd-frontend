    ...
    <search-card title="Search Role">
        <template v-slot:formInput="pObject">
            ...
            <div class="col-sm-12 col-md-4 col-lg-4 mb-2">
                ...
                <button class="btn btn-primary m-1" @keyup.enter="searchRole" @click.prevent="searchRole">Search</button>
                <button class="btn btn-danger m-1">Refresh</button>
                </div>
            </div>
        </template>
    </search-card>
    <div class="d-flex justify-content-end">
        <button @click="addRole()" type="button" class="btn btn-info btn-rounded mb-2">
            Add New Role
        </button>
    </div>
    <div class="">
        <div class="table-responsive p-0 m-0 border border-primary">
            <table id="smptable" class="table table-bordered table-hover">
                <thead>
                    <tr>
                        <th class="text-center bg-primary text-white p-2 m-0" style="width:10%;">ID</th>
                        <th class="text-center bg-primary text-white p-2 m-0">Role Description</th>
                        <th class="text-center bg-primary text-white p-2 m-0">Created at</th>
                        <th class="text-center bg-primary text-white p-2 m-0">Updated at</th>
                        <th class="text-center bg-primary text-white p-2 m-0" style="width:10%;">Update</th>
                        <th class="text-center bg-primary text-white p-2 m-0" style="width:10%;">Delete</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="r in roles" :key="r.id">
                        <td class="text-center align-middle fw-bold p-1 m-0">{{ r.id }}</td>
                        <td class="text-center align-middle fw-bold p-1 m-0">{{ r.role }}</td>
                        <td class="text-center align-middle fw-bold p-1 m-0">{{ r.created_at }}</td>
                        <td class="text-center align-middle fw-bold p-1 m-0">{{ r.updated_at }}</td>
                        <td class="text-center align-middle fw-bold p-1 m-0"><button @click="updateRole(r)" class="btn btn-info m-1">Update</button></td>
                        <td class="text-center align-middle fw-bold p-1 m-0"><button @click="deleteRole(r)" class="btn btn-danger m-1">Delete</button></td>
                    </tr>
                    <tr v-if="!roles.length && !isLoadingRoles ">
                        <td class="text-center align-middle fw-bold p-1 m-0" colspan="6">No records found.</td>
                    </tr>
        ...
        <pagination v-if="!isLoadingRoles" :data="paginationData" @update:currentPage="updateCurrentPage" />
    </div>
    </div>
  </div>
 <modal-sm :details="modalDetails" @close-modal="modalDetails.show = false">
    <form class="mt-2">
        <div class="mb-3" v-if="formModal.id != 0">
            ...
            <input type="text" v-model="formModal.id" class="form-control form-control-sm w-100 custom-font" id="recipient-name1" disabled/>
        </div>
        <div class="mb-3">
            <label for="recipient-name" class="control-label">Role:</label>
            <input type="text" v-model="formModal.role" class="form-control form-control-sm w-100 custom-font" :class="{ 'is-invalid': flagvl == true && !formModal.role }" id="recipient-name1"/>
            <div v-if="flagvl == true && !formModal.role" class="invalid-feedback">Please input role description</div>
        </div>
        <div class="d-flex justify-content-end">
            <button type="submit" @click.prevent="saveRole()" :disabled="flagvl == true" class="btn btn-success m-1">Save</button>
            <button @click.prevent="modalDetails.show = false" class="btn btn-danger m-1">Cancel</button>
        </div>
    </form>
 </modal-sm>
